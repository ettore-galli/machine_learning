...
def title_case_five_words(text: str) -> str:
    ...
    trimmed = (
        content[:5] if len(content) >= 5 else (content + ["Ideas"] * (5 - len(content)))
    )
...
def process_topic(topic: str):
    ...
    summary = pipe(p1, do_sample=False, max_new_tokens=40)
    ...
    raw_title = pipe(p2, do_sample=False, max_new_tokens=10)
    ...
    raw_tags = pipe(p3, do_sample=False, max_new_tokens=12)
